		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			GetCtrlVal(panelHandle,PANEL_SAMPLE_TIME,&SampleTime);  
			sprintf(strTemp,"0x01|%d",SampleTime); 
...
void displayData(char *indata)
{
	...
	//显示波形
	PlotWaveform (panelHandle, PANEL_GRAPH, Wave, 1024, VAL_FLOAT, 1.0, 0.0, 0.0, 1.0, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1, VAL_RED);
...
int CVICALLBACK SampleTimeChangedCallback (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	unsigned short SampleTime=0;
	char strTemp[50]={0};
	int ret;
	double displayTime=0,amp=1;
	switch (event)
	{
		case EVENT_COMMIT:
			//获取采样间隔时间并重新初始化设备
			...
			//displayTime = amp*(SampleTime*1024.0)/1000000.0;
			//SetCtrlAttribute(panelHandle,PANEL_TIMER_MEASURE,ATTR_INTERVAL,displayTime);
			break;
	}
	return 0;
}
