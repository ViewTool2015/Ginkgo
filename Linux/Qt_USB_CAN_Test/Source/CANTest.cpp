...
{
    int ReadDataNum;
    int DataNum = VCI_GetReceiveNum(VCI_USBCAN2, DevIndex, CANIndex);
    VCI_CAN_OBJ	*pCAN_ReceiveData = (VCI_CAN_OBJ *)malloc(Len*sizeof(VCI_CAN_OBJ));
    if((DataNum > 0)&&(pCAN_ReceiveData != NULL)){
        ReadDataNum = VCI_Receive(VCI_USBCAN2, DevIndex, CANIndex, pCAN_ReceiveData, DataNum);
...
int main(void)
{
    ...
    sleep(0.01);
            ...
            else
            {
                ...
                {
                    case CAN_BUS_STATUS_NO_ERROR:
                        printf("No Error\n");
                        break;
                    case CAN_BUS_STATUS_STUFF_ERROR:
                        printf("Stuff Error\n");
                        break;
                    case CAN_BUS_STATUS_FORM_ERROR:
                        printf("Form Error\n");
                        break;
                    case CAN_BUS_STATUS_ACKNOWLEDGEMENT_ERROR:
                        printf("Acknowledgment Error\n");
                        break;
                    case CAN_BUS_STATUS_BIT_RECESSIVE_ERROR:
                        printf("Bit recessive Error\n");
                        break;
                    case CAN_BUS_STATUS_BIT_DOMINANT_ERROR:
                        printf("Bit dominant Error\n");
                        break;
                    case CAN_BUS_STATUS_CRC_ERROR:
                        printf("CRC Error\n");
                        break;
                    case CAN_BUS_STATUS_SET_BY_SOFTWARE:
