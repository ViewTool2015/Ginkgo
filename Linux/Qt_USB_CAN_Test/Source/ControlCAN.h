/**
  ...
  * @brief   Ginkgo USB-CAN adapter low level controller API definition
  ******************************************************************************
  * @attention
  *
...
enum {
   CAN_BUS_STATUS_NO_ERROR, 
   CAN_BUS_STATUS_STUFF_ERROR, 
   CAN_BUS_STATUS_FORM_ERROR, 
   CAN_BUS_STATUS_ACKNOWLEDGEMENT_ERROR, 
   CAN_BUS_STATUS_BIT_RECESSIVE_ERROR, 
   CAN_BUS_STATUS_BIT_DOMINANT_ERROR, 
   CAN_BUS_STATUS_CRC_ERROR, 
   CAN_BUS_STATUS_SET_BY_SOFTWARE  
}; 

//1.Compatible with other CAN adapter datatype
...
typedef  struct  _VCI_BOARD_INFO_EX{
	uint8_t		ProductName[32];	//hardware name£¬for example: ¡°Ginkgo-CAN-Adaptor\0¡±£¨note: include string null end¡®\0¡¯£©
	uint8_t		FirmwareVersion[4];	//firmware version
	uint8_t		HardwareVersion[4];	//hardware version
	uint8_t		SerialNumber[12];	//adatper serial number
...
typedef  struct  _VCI_CAN_OBJ{
	...
	uint8_t	TimeFlag;	// if using timestamp£¬1: use TimeStamp, 0£ºnot use. TimeFlag and TimeStamp is available when the frame is recived frame
...
typedef struct _INIT_CONFIG_EX
{
	...
	uint8_t	CAN_Mode;	//CAN working mode. 0: normal£¬1: loopback£¬2: silent£¬3: silent loopback
	...
	uint8_t	CAN_RELAY;	//relay feature enable£¬0x00: close relay function£¬0x10: relay from CAN1 to CAN2£¬0x01: relay from CAN2 to CAN1£¬0x11: bidirectionaly relay
	uint32_t	Reserved;	//reserved
