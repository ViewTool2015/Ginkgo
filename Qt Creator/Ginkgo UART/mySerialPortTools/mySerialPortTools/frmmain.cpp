...
{    
    ...
    comList<<"ttyUSB0"<<"ttyUSB1"<<"ttyUSB2"<<"ttyUSB3"<<"ttyUSB4"<<"ttyUSB5"
         ...
         <<"ttyS7"<<"ttyS8"<<"ttyS9";
#endif
    int ret = UART_ScanDevice(1);
    for(int i=0;i<ret;i++){
        comList.append("VTCOM"+QString().number(i+1)+"-"+"CH0");
        comList.append("VTCOM"+QString().number(i+1)+"-"+"CH1");
    }
...
void frmMain::ReadMyCom()
{
    ...
    if (!ui->cboxPortName->currentText().contains("VTCOM")) {
        //这个判断尤为重要,否则的话直接延时再接收数据,空闲时和会出现高内存占用
        if (myCom->bytesAvailable()<=0){return;}
    }else{
        deviceIndex = portName.mid(5,1).toInt(NULL,0)-1;
        ChannelIndex = portName.mid(9,1).toInt(NULL,0);
    }
    ...
    if (!ui->cboxPortName->currentText().contains("VTCOM")) {
        ...
        uint8_t dataBuff[10240];
        uint16_t dataLen = 0;
        UART_ReadBytes(deviceIndex,ChannelIndex,dataBuff,&dataLen);
        if(dataLen > 0){
            buffer.append((const char*)dataBuff,dataLen);
        }else{
            return;
        }
    }
...
void frmMain::WriteMyCom()
{
    ...
    if (IsHexSend)//转化为16进制发送
    {
        ...
        if (!ui->cboxPortName->currentText().contains("VTCOM")) {
            ...
            int ret = UART_WriteBytes(deviceIndex,ChannelIndex,(uint8_t *)outData.data(),outData.length());
            if(ret != ERR_SUCCESS){
...
void frmMain::on_btnOpen_clicked()
{
    ...
    {
        if(portName.contains("VTCOM")){
            int ret;
            int deviceIndex = portName.mid(5,1).toInt(NULL,0)-1;
            int ChannelIndex = portName.mid(9,1).toInt(NULL,0);
            ret = UART_OpenDevice(deviceIndex,0);
            if(ret != ERR_SUCCESS){
                QMessageBox::warning(this,"警告","打开设备失败！");
                return;
            }
            UART_INIT_CONFIG UART_Config;
            UART_Config.StopBits = ui->cboxStopBit->currentIndex();
            if(ui->cboxParity->currentText()=="无"){
                UART_Config.Parity = 0;
            }else if(ui->cboxParity->currentText()=="奇"){
                UART_Config.Parity = 6;
            }else if(ui->cboxParity->currentText()=="偶"){
                UART_Config.Parity = 4;
            }
            UART_Config.WordLength = ui->cboxDataBit->currentText().toInt(NULL,0);
            UART_Config.BaudRate = ui->cboxBaudRate->currentText().toInt(NULL,0);
            UART_Config.RS485Mode = 485;
            ret =UART_InitDevice(deviceIndex,ChannelIndex,&UART_Config);
            if(ret != ERR_SUCCESS){
...
void frmMain::on_cboxPortName_currentIndexChanged(const QString &arg1)
{
    QStringList baudList;//波特率
    QStringList parityList;//校验位
    QStringList dataBitsList;//数据位
    if(arg1.contains("VTCOM")){
        ...
        ui->cboxDataBit->addItems(dataBitsList);
        ui->cboxDataBit->setCurrentIndex(0);
    }
}
